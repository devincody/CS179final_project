...
void Layer::set_style_flag(int flag){
    style_transfer = flag;
}

void Layer::print_weight(int pos, int numb) const{
    float *temp = new float[numb]();
    CUDA_CALL(cudaMemcpy(temp, &(weights[pos]), numb*sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < numb; i++){
        std::cout << temp[i] <<", ";
    }
    std::cout << std::endl;

    CUDA_CALL(cudaMemcpy(temp, &(biases[pos]), numb*sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < numb; i++){
        std::cout << temp[i] <<", ";
    }
    std::cout << std::endl;
...
void Layer::print_input_tensor(int pos, int numb) const{
    float *temp = new float[numb]();
    CUDA_CALL(cudaMemcpy(temp, &(in_batch[pos]), numb*sizeof(float), cudaMemcpyDeviceToHost));
    for (int i = 0; i < numb; i++){
        std::cout << temp[i] <<", ";
    }
    std::cout << std::endl;
...
void Layer::allocate_buffers()
{
    ...
    //TODO (final):
    int n_metrics = 0;
    if(!layer_name.compare(0,4, "bloc")){
        if(!layer_name.compare(7,4, "conv")){
            if (!layer_name.compare(11,1, "1")){
                n_metrics = n*n; //space for a gram matrix here
            }

            if(!layer_name.compare(0,11, "block5_conv2")){
                n_metrics = out_size; // space for all features here
            }
            CUDA_CALL(cudaMalloc(&loss_metric, n_metrics*sizeof(float)));
        }
    }
    /************************************************************/
...
void Layer::init_weights_biases()
{
    ...
    if(this->init_randomly){
        ...
        if (!layer_name.compare(7,4,"dens")){
            // IF DENSE
            std::cout << "n_weights: " << n_weights << " n_biases: " << n_biases << std::endl;
            int in_size = n_weights/n_biases;

            get_weights(layer_name, n_biases, in_size, 1, 1, h_weights);
            get_bias(layer_name, n_biases, h_biases);
        }
        else{
            // IF NOT DENSE
            std::cout << "Not dense "<< std::endl;
            get_weights(layer_name, n_biases, c, 3, 3, h_weights);
            get_bias(layer_name, n_biases, h_biases);
        }
        ...
        if (h_weights){
            ...
            std::cout << "h_weights dne" << std::endl;
        }

        if (h_biases){ 
            CUDA_CALL(cudaMemcpy(biases,  h_biases,  n_biases *sizeof(float), cudaMemcpyHostToDevice));
        } else {
            std::cout << "h_biases dne" << std::endl;
        }
...
: Layer(prev, cublasHandle, cudnnHandle, layer_name, init_rand)
{
    ...
    std::cout << "dense weights, biases, c*h*w: " << n_weights << " " << n_biases << " " << c*h*w << " "  << std::endl;
...
void Conv2D::forward_pass()
{
    ...
    } else if (style_transfer == 1){
        // SAVE CONTENT loss
        if (!layer_name.compare(11,1, "1")){
            // if first conv layer in a block
            this->gram(out_batch, n, c*h*w, loss_metric);
        }
	...
	if (style_transfer == 2){
        if(!layer_name.compare(0,11, "block5_conv2")){
            ...
            CUBLAS_CALL(cublasSaxpy(cublasHandle, out_size,
                                    &minus_one,
                                    loss_metric, 1,
                                    out_batch, 1)); // out_batch -= loss_metric

            CUBLAS_CALL(cublasScopy(cublasHandle, out_size,
                                    out_batch, 1,
                                    grad_out_batch, 1)); // grad_out_batch = out_batch

        } else if (!layer_name.compare(11,1, "1")){
            /* Name of layer must be: blockX_conv1
            Purpose of this block of code: Add style losses to 
            dL/dF = 1/(out_size)^2 ((G-A)^T F)
            G is the feature gram matrix
            A is the "style" gram matrix stored in loss_metric
            F is the n by c*w*h feature matrix 
            */
            float minus_one = -1;

            float *d_gram;
            CUDA_CALL(cudaMalloc(&d_gram, n*n*sizeof(float)));
            this->gram(out_batch, n, c*w*h, d_gram); // create G

            CUBLAS_CALL(cublasSaxpy(cublasHandle, n*n,
                                    &minus_one,
                                    loss_metric, 1,
                                    d_gram, 1)); // G -= A

            float scaling = 1.0/(out_size*out_size);
            float zero = 0;
            CUBLAS_CALL(cublasSgemm(cublasHandle, CUBLAS_OP_T, CUBLAS_OP_N,
                                    n, c*w*h, n,
                                    &scaling,
                                    ...
                                    out_batch, n,
                                    &zero,
                                    grad_out_batch, n)); // grad_out_batch = 1/scaling (F)G^T + 0*grad_out_batch

            CUDA_CALL(cudaFree(d_gram));
...
: Layer(prev, cublasHandle, cudnnHandle, layer_name, init_rand)
{
    ...
    //std::cout << "debug2: " << stride << " sdf " << init_rand << " ddd " << padding << std::endl;
    //std::cout << "layer name: "<< layer_name << " aaa " << mode << " nnn " << prev << std::endl;
