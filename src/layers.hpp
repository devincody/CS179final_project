...
public:
    ...
    void print_weight(int pos, int numb) const;
    void print_input_tensor(int pos, int numb) const;

    void set_style_flag(int flag);
protected:
    ...
    int style_transfer = -1;//-1=> normal operation
                            //0 => save content loss
                            //1 => save style   loss
                            //2 => transfer style
...
public:
    ...
    void forward_pass() override;
    void backward_pass(float learning_rate) override;
    void gram(float* in, int n, int cwh, float* gram);
private:
